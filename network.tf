...
# 2. Das Subnetz (Das Zimmer für unseren Server)
resource "opentelekomcloud_vpc_subnet_v1" "ai_subnet" {
  ...
  # OTC Standard DNS Server + Google Fallback, 
  # damit unsere Instanz später nach draußen ins Internet telefonieren kann (z.B. um Modelle zu laden)
  dns_list = ["100.125.4.25", "8.8.8.8"]
}

# 3. Die öffentliche IP Adresse (Elastic IP)
